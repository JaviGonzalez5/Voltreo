...
class TestGroupsPlayInParallel:
    ...
    def test_nocturno_wave_then_mixed_groups_stay_interleaved_by_slot(self):
        # Caso tipo Bahia Nocturno:
        # tanda 1 = masculino + femenino; tanda 2 = mixto.
        # El mixto no debe jugar primero todo Grupo A y luego todo Grupo B.
        masc3 = "masculino:3a"
        ...
        mix3 = "mixto:3a"
        mix35 = "mixto:35a"
        divisions = [masc3, masc35, fem3, fem35, mix3, mix35]
        waves = {
            masc3: 1, masc35: 1, fem3: 1, fem35: 1,
            mix3: 2, mix35: 2,
        }
        pairs = [
        ...
        cfg = TournamentConfig(
            name="Bahia Nocturno",
            start_date=date(2026, 6, 12), end_date=date(2026, 6, 12),
            divisions=divisions, division_waves=waves,
            format=TournamentFormat.GROUPS, group_size=4,
            courts=[TournamentCourt(id=f"c{i}", name=f"Pista {i}") for i in range(1, 5)],
            pairs=pairs, match_duration_minutes=15, rest_between_matches_min=15,
            day_start_time=time(19, 0), day_end_time=time(2, 0),
            division_draws=[
                TournamentDivision(key=division, format=TournamentFormat.GROUPS, group_size=4)
                for division in divisions
            ],
        )
        cfg = schedule_tournament(generate_tournament_structure(cfg))
        group_name = {g.id: g.name for g in cfg.groups}
        scheduled = [m for m in cfg.matches if m.round == MatchRound.GROUP and m.match_date]

        def minute(m, attr):
            value = getattr(m, attr)
            return (value.hour + 24 if value.hour < 8 else value.hour) * 60 + value.minute

        wave1 = [m for m in scheduled if waves[m.division] == 1]
        wave2 = [m for m in scheduled if waves[m.division] == 2]
        assert max(minute(m, "end_time") for m in wave1) <= min(
            minute(m, "start_time") for m in wave2
        )

        from collections import defaultdict
        by_slot = defaultdict(list)
        for match in wave2:
            by_slot[(match.match_date, match.start_time)].append(match)

        for slot_matches in by_slot.values():
            if len(slot_matches) < 4:
                continue
            assert len({m.division for m in slot_matches}) >= 2
            assert len({group_name.get(m.group_id) for m in slot_matches}) >= 2
            assert max(
                sum(
                    1 for other in slot_matches
                    if other.division == match.division and other.group_id == match.group_id
                )
                for match in slot_matches
            ) <= 2

    def test_unbalanced_wave_does_not_fill_full_row_with_group_a_when_options_exist(self):
        # Reproduce una tanda con categorias descompensadas: algunas solo tienen
        # Grupo A, pero otras tienen Grupo A/B/C. Si existen alternativas B/C en
        # la misma hora, no queremos una fila completa de Grupo A.
        masc3 = "masculino:3a"
        masc35 = "masculino:35a"
        fem3 = "femenino:3a"
        fem35 = "femenino:35a"
        ...
        pair_counts = {
            masc3: 9,    # 3 grupos de 3
            masc35: 10,  # grupos desiguales
            fem3: 5,     # solo Grupo A
            fem35: 6,    # solo Grupo A
        }
        group_sizes = {
            masc3: 3,
            masc35: 4,
        ...
        pairs = [
            _pair(f"{division[-3:]}-{idx:02d}", division)
            for division in divisions
            for idx in range(pair_counts[division])
        ]
        cfg = TournamentConfig(
            name="Bahia Tanda 1",
            start_date=date(2026, 6, 12), end_date=date(2026, 6, 12),
            ...
            division_draws=[
                TournamentDivision(
                    key=division,
                    format=TournamentFormat.GROUPS,
                    group_size=group_sizes[division],
                )
                for division in divisions
            ],
        )
        ...
        for match in cfg.matches:
            if match.round == MatchRound.GROUP and match.match_date:
                by_slot[(match.match_date, match.start_time)].append(match)
        ...
        ordered_slots = sorted(
            ...
            key=lambda slot: (
                slot[0],
                (slot[1].hour + 24 if slot[1].hour < 8 else slot[1].hour),
                slot[1].minute,
            ),
        )
        for slot in ordered_slots:
            slot_matches = by_slot[slot]
            if len(slot_matches) < 4:
                continue
            groups = [group_name.get(match.group_id) for match in slot_matches]
            if Counter(groups)["Grupo A"] == 4:
                future_other_groups = [
                    match
                    for match in cfg.matches
                    if match.round == MatchRound.GROUP
                    and group_name.get(match.group_id) != "Grupo A"
                    and match.match_date
                    and (
                        match.match_date,
                        (match.start_time.hour + 24 if match.start_time.hour < 8 else match.start_time.hour),
                        match.start_time.minute,
                    ) > (
                        slot[0],
                        (slot[1].hour + 24 if slot[1].hour < 8 else slot[1].hour),
                        slot[1].minute,
                ...
                assert not future_other_groups, (
                    "No debe haber una fila completa de Grupo A mientras queden "
