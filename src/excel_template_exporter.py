...
from .ranking_scorer import standings_by_group, ScoringRules
...
def export_groups_to_template(
    ...
    _write_sheet_clasificacion(wb, phase)
...
def _write_sheet_clasificacion(wb: Workbook, phase: RankingPhase) -> None:
    """Hoja de clasificación por grupos (solo si hay resultados registrados)."""
    ...
    headers = ["#", "Pareja", "PJ", "G", "E", "P", "Sets G", "Sets C", "Dif Sets", "Jgs G", "Jgs C", "Dif Jgs", "Pts"]
    ...
    for gid, table in by_group.items():
        # Cabecera de grupo
        c = ws.cell(row=row_i, column=1, value=group_label.get(gid, gid))
        c.font = _font(bold=True, color=C_HDR_FG, size=10)
        c.fill = _fill(C_GROUP)
        ws.merge_cells(start_row=row_i, start_column=1, end_row=row_i, end_column=len(headers))
        c.alignment = _align("left", "center")
        ws.row_dimensions[row_i].height = 18
        row_i += 1

        for pos, s in enumerate(table, 1):
            medal = {1: "1°", 2: "2°", 3: "3°"}.get(pos, f"{pos}°")
            row_vals = [
                medal, s.pair_name,
                s.played, s.won, s.drawn, s.lost,
                s.sets_for, s.sets_against, s.set_diff,
                s.games_for, s.games_against, s.game_diff,
                s.points,
            ]
            medal_bg = {1: C_GOLD, 2: C_SILVER, 3: C_BRONZE}.get(pos)
            row_bg = "F2F2F2" if pos % 2 == 0 else "FFFFFF"
            for ci, val in enumerate(row_vals, 1):
                c = ws.cell(row=row_i, column=ci, value=val)
                c.border = _border()
                c.font = _font(bold=(ci == len(headers)), size=9)
                c.alignment = _align("left" if ci == 2 else "center", "center")
                if medal_bg and ci == 1:
                    c.fill = _fill(medal_bg)
                else:
                    c.fill = _fill(row_bg)
            ws.row_dimensions[row_i].height = 15
            row_i += 1

        row_i += 1  # espacio entre grupos
